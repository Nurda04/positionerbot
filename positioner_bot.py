...
@dp.message_handler(commands=['at', 'go'])
async def cmd_set_position(message: types.Message):
    if not db.db_check_registr(message.from_user.id):
        ...
        global operation
        cmd_variants = ('/at', '/go')
        operation = 'at' if message.text.startswith(cmd_variants[0]) else 'go'
        pos = message.text
        for i in cmd_variants:
            pos = pos.replace(i, '').strip()
        if not pos or pos == "@positionerbot":
            text = "Укажите ваше местоположение" if operation == 'at' else 'Укажите ваше направление'
            await ProfileStatesGroup.set_position.set()
        else:
            text = f"Местоположение {pos} записано!" if operation == 'at' else f"Направление {pos} записано!"
...
async def cmd_get_position(message: types.Message):
    ...
    if not pos_name or pos_name == "@positionerbot":
        ...
        text = f"Нет пользователя по имени {pos_name}" if not check_name else get_position2(pos_name)
    await message.reply(text)
